...
def TestJet(args, model, data_loader_target, modelIdx,amount):
    ...
    with torch.no_grad():
        with open(args.results_path + "/txt/misclassified.txt","a+") as mis_f:
            ...
            for (img_paths, catimages, labels) in data_loader_target:
                ...
                #images = catimages
                # if args.mode == "train":
                label_pred  = model(images) #呼叫forward
                # if args.mode == "test":
                #     label_pred  = detector.predict(images) #呼叫forward
